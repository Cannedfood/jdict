...
use crate::{
    kanjidic::{
        Character, Codepoint, DicRef, Header, Kanjidic, Meaning, Misc, OnType, QueryCode,
        QueryCodeType, Radical, Reading, ReadingMeaningGroup, ReadingType, SkipMisclass, Variant,
        VariantType,
    },
    util::read_file,
};
    ...
    for child in element.children().filter(|n| n.is_element()) {
        match child.tag_name().name() {
            ...
            "reading_meaning" => character
                ...
                .push(parse_reading_meaning(&child)),
            _ => panic!(
                "Unknown element <{}> in <character>",
                child.tag_name().name()
            ),
        }
    }
...
fn parse_reading_meaning(element: &Node) -> ReadingMeaningGroup {
    ...
    for child in element.children().filter(|n| n.is_element()) {
        match child.tag_name().name() {
            "rmgroup" => {
                for rmgroup_child in child.children().filter(|n| n.is_element()) {
                    match rmgroup_child.tag_name().name() {
                        ...
                        _ => panic!(
                            "Unknown element <{}> in <rmgroup>",
                            rmgroup_child.tag_name().name()
                        ),
                    }
                }
            }
            ...
            _ => panic!(
                "Unknown element <{}> in <reading_meaning>",
                child.tag_name().name()
            ),
        }
    }
...
fn parse_reading(element: &Node) -> Reading {
    Reading {
        ...
        typ: element
            .attribute("r_type")
            .map_or(ReadingType::ja_kun, |r_type| match r_type {
        ...
        on_type: element
            .attribute("on_type")
            .map_or(OnType::none, |on_type| match on_type {
                "kan" => OnType::kan,
                "go" => OnType::go,
                "tou" => OnType::tou,
        ...
        approved_for_joyou_kanji: element
            .attribute("r_status")
            .map_or(false, |r_status| r_status == "jy"),
    }
}
...
fn parse_codepoint(element: &Node) -> Codepoint {
    ...
    for child in element.children().filter(|n| n.is_element()) {
        match child.tag_name().name() {
            "cp_value" => match child
                .attribute("cp_type")
                .expect("cp_value with missing cp_type attribute")
            {
    ...
    for child in element.children().filter(|n| n.is_element()) {
        match child.tag_name().name() {
            "rad_value" => {
                match child
                    .attribute("rad_type")
                    .expect("rad_value with missing rad_type attribute")
                {
                    "classical" => {
                        result.classical = child
                            .text()
                            .unwrap()
                            .to_string()
                            .parse()
                            .expect("rad_value content cannot be parsed into a u16")
                    }
                    "nelson_c" => {
                        result.nelson_c = child
                            .text()
                            .unwrap()
                            .to_string()
                            .parse()
        ...
        match child.tag_name().name() {
            "grade" => {
                result.grade = child
                    ...
                    .to_string()
                    .parse()
                    .expect("grade content cannot be parsed into a u8")
            }
            "stroke_count" => result.stroke_count.push(
                child
                    .text()
                    .unwrap()
                    .to_string()
                    .parse()
                    .expect("stroke_count content cannot be parsed into a u8"),
            ),
            ...
            "freq" => {
                result.freq = child
                    ...
                    .to_string()
                    .parse()
                    .expect("freq content cannot be parsed into a u16")
            }
            ...
            "jlpt" => {
                result.jlpt = child
                    ...
                    .to_string()
                    .parse()
                    .expect("jlpt content cannot be parsed into a u8")
            }
            ...
            "dic_ref" => {
                result.push(DicRef {
                    index_number: child
                        ...
                        .to_string()
                        .parse()
                        .expect("dic_ref content cannot be parsed into a u16"),
                    moro_volume: child
                        .attribute("m_vol")
                        .map(|v| v.parse().expect("m_vol content cannot be parsed into a u8"))
                        .unwrap_or(0),
                    moro_page: child
                        .attribute("m_page")
                        .map(|v| {
                            v.parse()
                                .expect("m_page content cannot be parsed into a u16")
                        })
                        .unwrap_or(0),
                    typ: child
                        .attribute("dr_type")
                        .expect("dic_ref with missing dr_type attribute")
                        .parse()
                        .unwrap(),
                });
            }
            _ => panic!(
                "Unknown element <{}> in <dic_number>",
                child.tag_name().name()
            ),
        }
    }
...
fn parse_variant(element: &Node) -> Variant {
    Variant {
        ...
        typ: match element
            .attribute("var_type")
            .expect("variant with missing var_type attribute")
        {
            ...
            _ => panic!(
                "Unknown var_type: {}",
                element.attribute("var_type").unwrap()
            ),
        },
    }
}
