...
impl Query {
    ...
    pub fn starts_with(s: &str) -> Self {
        Self::new(QueryType::StartsWith, s)
    }
    pub fn contains(s: &str) -> Self {
        Self::new(QueryType::Contains, s)
    }
    pub fn ends_with(s: &str) -> Self {
        Self::new(QueryType::EndsWith, s)
    }
    pub fn exactly(s: &str) -> Self {
        Self::new(QueryType::Exactly, s)
    }
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            word if word.starts_with('*') && word.ends_with('*') => {
                Ok(Query::contains(&word[1..word.len() - 1]))
            }
...
impl FullTextIndex {
    ...
    pub fn optimize(&mut self) {
        self.entries
            .values_mut()
            .for_each(FullTextIndex::dedup_weighted);
            ...
            QueryType::StartsWith => {
                result.extend(
                    self.entries
                        ...
                        .flat_map(|(key, value)| {
                            let match_quality = EXACT_MATCH_BONUS * (key == q) as i32
                                - LENGTH_PENALTY * (key.len() as i32 - q.len() as i32);

                            value
                                .iter()
                                .map(move |(id, weight)| (*id, *weight + match_quality + base))
                        }),
                );
            }
            QueryType::EndsWith => {
...
fn split_words(text: &str) -> impl Iterator<Item = &str> + '_ {
    const STOP_WORDS: &[&str] = &[
        "out", "as", "esp", "up", "person", "from", "at", "an", "by", "be", "g", "or", "on",
        "with", "for", "e", "etc", "one", "and", "s", "in", "the", "a", "of", "to", "not", "it",
        "is",
    ];
